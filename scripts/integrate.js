...
/**
 * design-comments - Auto Integration Script
 * 
...
try {
  ...
  console.error('   These should be included with design-comments package.');
...
function generateFiles(config) {
  ...
  let envContent = `# design-comments Configuration
...
`;
  ...
  let envServerContent = `# design-comments - Server Secrets
...
/**
 * If an older integration added GitHub/Jira routes but not the summarize proxy, append it before `return middlewares`.
 */
function patchWebpackSummarizeProxy(webpackContent) {
  if (webpackContent.includes('/api/discussions-summarize')) {
    return webpackContent;
  }
  const anchor = 'Unhandled jira-issue proxy error';
  const idx = webpackContent.indexOf(anchor);
  if (idx === -1) {
    return webpackContent;
  }
  const needle = '});\n\n      return middlewares;';
  const tail = webpackContent.indexOf(needle, idx);
  if (tail === -1) {
    return webpackContent;
  }
  const insertPos = tail + '});'.length;
  ...
  return webpackContent.slice(0, insertPos) + summarizeBlock + webpackContent.slice(insertPos);
}
...
function integrateWebpackMiddleware() {
  ...
  const hasOAuth = webpackContent.includes('/api/github-oauth-callback');
  const hasJira = webpackContent.includes('/api/jira-issue');
  const hasSummarize = webpackContent.includes('/api/discussions-summarize');

  if ((hasOAuth || hasJira) && !hasSummarize) {
    const patched = patchWebpackSummarizeProxy(webpackContent);
    if (patched !== webpackContent) {
      fs.writeFileSync(webpackDevPath, patched);
      console.log('   ✅ Added /api/discussions-summarize proxy to webpack.dev.js');
      webpackContent = patched;
    }
  }

  // Check if already integrated (including summarize)
  if (webpackContent.includes('/api/github-oauth-callback') || webpackContent.includes('/api/jira-issue')) {
    if (webpackContent.includes('/api/discussions-summarize')) {
      ...
      console.log('   ⚠️  webpack.dev.js already appears to have commenting system integration (summarize proxy not added — check Jira block anchor)');
    }
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(cwdPackageJson)) {
        ...
        if (packageJson.name === 'design-comments') {
          return packageJson.version || 'unknown';
        }
      }
    } catch (e2) {
  ...
  try {
    ...
    traverse(ast, {
      ImportDeclaration(path) {
        ...
        if (source.includes('commenting-system') || source.includes('@app/commenting-system') || source.includes('design-comments')) {
    ...
    if (!hasCommentImport) {
      ...
      const providerImports = types.importDeclaration(
        ...
        types.stringLiteral('design-comments')
      );
...
function modifyAppLayoutTsx(filePath) {
  ...
  try {
    ...
    traverse(ast, {
      ImportDeclaration(path) {
        const source = path.node.source.value;
        if (source.includes('design-comments')) {
...
async function main() {
  ...
  const title = `design-comments (v.${version})`;
  ...
  // Welcome & Explanation
  console.log('🚀 Welcome to design-comments!\n');
  ...
  if (hasProject.hasProject === 'no') {
    ...
    console.log('   npx design-comments init\n');
    rl.close();
    return;
  }
  ...
  if (setupJira.setup) {
    ...
    console.log('4. Give it a name (e.g., "design-comments")');
